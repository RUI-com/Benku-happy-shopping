...
const Sidebar = () => {
  ...
  `}>
    ...
    overflow-y-auto overflow-hidden border-b '>
      {cart.map(item => {
    ...
    mt-4 '>
      ...
      {/* clear cart icon */}
      <div onClick={clearCart} className='cursor-pointer py-4 bg-[#f48b62] text-white w-12 h-12 rounded-lg flex justify-center
      items-center text-xl '>
        <FiTrash2/>
      </div>
      </div>
      <Link to='/' className='bg-gray-200 rounded-md flex p-4 justify-center items-center
      text-primary w-full font-medium '>View cart</Link>
      <Link to='/' className='bg-[#FB5887] rounded-md flex p-4 justify-center items-center
      text-[#fff] w-full font-medium '>Checkout</Link>
    </div>
